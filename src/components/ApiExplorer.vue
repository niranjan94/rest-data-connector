<template>
  <b-container>
    ...
    <b-form @submit="onSubmit" @reset="onReset" autocomplete="off">
      <b-form-group
        :label="`${specInfo.title} API Spec URL`"
      ...
      >
        <b-input-group size="sm">
          ...
          <b-input-group-append>
            ...
            <b-btn type="reset" variant="outline" :disabled="specLoading"
              >&times;</b-btn
            >
          </b-input-group-append>
        </b-input-group>
    ...
    <data-view
      v-if="openDataView"
      :open-data-view.sync="openDataView"
      :result="result"
    />
    ...
    >
      Invalid API Spec. Please ensure the URL given points to a
      {{ specInfo.title }} compatible API specification.
    </b-alert>
        ...
        .then(
          ...
          (response) => {
            ...
            if (response.status > 0) {
              ...
              this.generalAlert =
                'Request failed. <a href="https://tableau.github.io/webdataconnector/docs/wdc_cors" target="_blank" style="color: inherit;"><small>Could be a CORS issue.</small></a>';
            }
          }
        );
    },
