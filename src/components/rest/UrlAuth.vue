<template>
  <div>
    <b-row>
      <b-col md="6">
        <b-form-group
          :label="directMode ? 'Endpoint:' : 'Base URL:'"
          label-for="baseUrl"
          label-size="sm"
          :description="
            directMode
              ? 'The endpoint to make a request to'
              : 'Base URL to use for all API calls'
          "
        >
          <b-form-input
            id="baseUrl"
            ...
            :placeholder="
              directMode
                ? 'https://example.com/api/v1/flights.json'
    ...
    <b-row
      v-if="
        ...
        config.authorizationMode === 'token'
      "
    >
      <b-col md="12">
        <b-form-group label="Token:" label-size="sm" label-for="token">
          <b-form-input
            id="token"
            type="text"
            size="sm"
    ...
    <b-row
      v-if="
        config.authorizationMode === 'basic' ||
        config.authorizationMode === 'expanded_token'
      "
    >
      <b-col md="6">
        <b-form-group label="Username:" label-size="sm" label-for="username">
          <b-form-input
            id="username"
            type="text"
            size="sm"
      ...
      <b-col md="6">
        <b-form-group
          :label="config.authorizationMode === 'basic' ? 'Password:' : 'Token:'"
          label-size="sm"
          label-for="password"
        >
          <b-form-input
