<template>
  <b-modal
    ...
    :ok-title="
      inTableau ? 'Load into Tableau' : 'Run connector from tableau to load'
    "
  >
    <div v-if="result.definition">
      <code>{{ result.definition.endpoint }}</code> -
      <span class="text-muted">{{ result.definition.summary }}</span>
      <hr />
    </div>

    <b-form @submit="onOkay">
      <b-form-group
        v-if="result.contentType === 'application/xml'"
        label="XML Parsing options"
        label-size="sm"
        label-for="parsingOptions"
      >
        <b-form-checkbox-group
          id="parsingOptions"
          name="parsingOptions"
          v-model="parsingOptions"
        >
          <b-form-checkbox value="ignoreAttributes"
            >Skip attributes</b-form-checkbox
          >
      ...
      <b-form-group
        ...
        label-size="sm"
        :description="
          pathDescription + ' <br> Preview the data below before proceeding.'
        "
      >
        <b-input-group size="sm">
          <b-form-input id="pathToExport" type="text" v-model="pathToExport">
          </b-form-input>
          <b-input-group-append>
            <b-btn
              :pressed="useJsonPath"
              variant="info"
              v-on:click="onPathTypeToggle"
              >JSON Path</b-btn
            >
            <b-btn
              :pressed="!useJsonPath"
      ...
      >
        ...
        >
          <b-form-checkbox value="flattenObject"
            >Flatten Object</b-form-checkbox
          >
        </b-form-checkbox-group>
      </b-form-group>
...
export default {
  ...
  data() {
    let connectorName = this.result.definition
      ? this.result.definition.summary
      : this.result.url;
  ...
  computed: {
    pathDescription() {
      return this.useJsonPath
        ? "Enter the <a href='http://goessner.net/articles/JsonPath/'>JSONPath</a> to load into Tableau (eg. <code>$.data</code>, <code>$.meta</code>). Leave blank to load everything."
        : "Enter the path to load into Tableau (eg. <code>root.data</code>, <code>root.meta</code>, <code>root</code>)";
    },
    parsedData() {
    ...
    treeViewOptions() {
      if (
        !this.parsedData ||
        keys(this.parsedData) > 10 ||
        this.parsedData.length > 10
      ) {
        return {
          maxDepth: 0,
        };
    ...
    dataToExportTreeViewOptions() {
      if (
        !this.dataToExport ||
        keys(this.dataToExport) > 10 ||
        this.dataToExport.length > 10
      ) {
        return {
          maxDepth: 0,
        };
