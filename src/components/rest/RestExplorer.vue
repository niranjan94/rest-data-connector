<template>
  <b-container>
    ...
    <not-tableau-alert />
    <url-auth
      :request-config.sync="requestConfig"
      :direct-mode="true"
      :with-method-picker="true"
    />
    <custom-headers :request-headers.sync="requestConfig.headers" />
    <body-editor
      :request-body.sync="requestBody"
    ...
    makeRequest() {
      ...
      switch (this.requestBody.type) {
        case "raw":
          ...
          break;
        case "query":
          this.requestBody.value.forEach(
            (item) => (body[item.name] = item.value)
          );
          break;
        case "form_data":
          body = new FormData();
          this.requestBody.value.forEach((item) =>
...
`;
          ...
          this.$notify({
            ...
            title:
              e.status <= 0
                ? 'Request failed due to <a href="https://tableau.github.io/webdataconnector/docs/wdc_cors" target="_blank" style="color: inherit;">possible CORS error.</a>'
                : "Request failed",
            text,
          });
        });
