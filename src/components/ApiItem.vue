    ...
    <b-collapse :id="uuid" accordion="my-accordion" role="tabpanel">
      <b-card-body>
        <b-form @submit="onSubmit" autocomplete="off">
          <table
            v-if="definition.parameters.length"
            class="table table-sm parameters-table"
          >
            <thead>
              <tr>
            ...
            <tbody>
              <tr
                v-for="(parameter, index) in definition.parameters"
                :key="index"
              >
      ...
      makeRequest(
        ...
        .catch((e) => {
          ...
          this.$notify({
            ...
            text: `<strong>Endpoint:</strong> ${
              this.definition.endpoint
            }<br><strong>Reason:</strong> ${getErrorMessage(e.body)}`,
          });
        });
    },
  },
